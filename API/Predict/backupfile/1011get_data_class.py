...
class get_predic_data(object):
    ...
    def get_Daily_coming_30days_vaule(self, predicArea, start_date):
        ...
        path = folder_prediction_path + 'result/%s/predict_%s_%s_%s_%s_daily' % (start_date, predicArea, start_year, start_month, start_day)
    ...
    def get_Monthly_latest_12months_daily_value(self, predicArea, start_year, start_month, temp_mode, sub_mode, start_date):
        path = folder_prediction_path + 'result/%d/past_%s_%d_%d_%d_T%d_S%d_daily' % (start_date, predicArea, start_year, start_month, 12, temp_mode, sub_mode)
        # file = '/home/uk/PredictionServer/prediction/result/predict_naju_2019_1_1_daily'
        print(path)
        if not os.path.isfile(path):
            return False
    ...
    def get_Monthly_latest_12months_monthly_value(self, predicArea, start_year, start_month, temp_mode, sub_mode,start_date):
        path = folder_prediction_path + 'result/%d/past_%s_%d_%d_%d_T%d_S%d_monthly' % (start_date, predicArea, start_year, start_month, 12, temp_mode, sub_mode)
        # file = '/home/uk/PredictionServer/prediction/result/predict_naju_2019_1_1_daily'
        print(path)
        if not os.path.isfile(path):
            return False
...
class get_train_model(object):
    ...
    def train_model_result(self, filename):
        ...
        # 모델 결과 읽기.
        file_path = folder_prediction2_path + 'result/%d/%s.csv' % (self._pkey, filename)
...
def model_create(filename, period_start, period_end, period_start_time, period_end_time, pkey):
    ...
    # print(final_result)
    if not os.path.isdir(folder_prediction2_path + 'result/%d' % (pkey)):
        os.makedirs(folder_prediction2_path + 'result/%d' % (pkey))
        pass

    print(folder_prediction2_path + 'result/%d/%s.csv' % (pkey, filename))

    with open(folder_prediction2_path + 'result/%d/%s.csv' % (pkey, filename), 'w') as f:
