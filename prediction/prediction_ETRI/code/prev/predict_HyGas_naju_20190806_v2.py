...
def estimate_insu_per_cat(area, cat, target_year, target_month, temp_mode):
...
def conduct_predict(area, start_year, start_month, month_range, temp_mode, sub_mode):
	...
	for ridx in range(1, month_range+1):
		...
		for cat in category:
			...
			est_insu, true_insu = estimate_insu_per_cat(area, cat, current.year, current.month, temp_mode)
