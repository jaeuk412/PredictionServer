...
def write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_true_insu_list, category_list, mode):
	...
	f = open('daily_prediction_gwangju_%d'%(target_year), 'a+')
...
def predict_temp(raw_data, target_year, target_mon):
	...
	if target_year == 2018 and target_mon in [12, 1, 2]:
		weight_list = [1, 1, 1, 0.3]
	elif target_year == 2019 and target_mon in [12, 1, 2]:
		weight_list = [1, 1, 0.3, 0.3]
	else:
		weight_list = [1, 1, 1, 1]
...
def predict_avg_insu_by_averaging(cat, raw_data, target_year, target_mon):
	...
	for tday in range(1, mon_days+1): 
		...
		if len(prev_avg_insu_target) > 0:
			prev_avg_insu.append( np.average(prev_avg_insu_target, weights=[x for x in np.linspace(1, 1+0.2*(len(prev_avg_insu_target)-1), len(prev_avg_insu_target))] ))
...
def predict_avg_insu(cat, raw_data, target_year, target_mon):
	...
	elif cat in ['houseCooking', 'houseCHeating', 'salesOne', 'bizHeating', 'bizCooling', 'industry', 'heatFacility', 'CNG']:

		return predict_avg_insu_by_averaging(cat, raw_data, target_year, target_mon)

	# curve fitting.
	elif cat in ['salesTwo', 'heatCombined']:
...
def conduct_predict(history_file_name, target_year):
	...
	gwangju_category = ['house', 'houseCooking', 'houseJHeating', 'houseCHeating', 'salesOne', 'salesTwo', 'bizHeating', 'bizCooling', 'industry', 'heatFacility', 'heatCombined', 'CNG']
	...
	if math.isnan(float(tmp_cat[0])) == False:
		...
		f = open('daily_prediction_gwangju_%s'%(target_year), 'w')
		# - write column name first
		data = "year month date "
		for tidx in range(len(gwangju_category)):
			data += "%s_pred %s_true "%(gwangju_category[tidx], gwangju_category[tidx])
		...
		for tmon in range(1,13):
			...
			write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_true_insu_list, gwangju_category, 1)
	...
	# when there is no true label of target_year (e.g., 2019)
		...
		data = "year month date "
		for tidx in range(len(gwangju_category)):
			data += "%s "%(gwangju_category[tidx])
		data += "pred_sum\n"
		f.write(data)
		f.close()

		# true monthly insu volume of Y2019 of gwangju area
		...
		for tmon in range(1,7):
			...
			# write prediction result into file
			write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_pred_insu_list, gwangju_category, 2)
...
conduct_predict('merged_gwangju_2014_2018', 2018)
