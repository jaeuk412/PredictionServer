...
def predict_temp(area, target_year, target_mon, temp_mode):
	...
	# return the measured temp
		...
		target_temp_data = pd.read_csv(folder_path+'data/temperature/%s_temp_%d'%(area, target_year), delim_whitespace=True)
...
def conduct_prediction(area, start_year, start_month, month_range, temp_mode, sub_mode, start_date):
	...
	daily_output_file = folder_path+'result/%d/past_%s_%d_%d_%d_T%d_S%d_daily'%(start_date, area, start_year, start_month, month_range, temp_mode, sub_mode)
	...
	monthly_output_file = folder_path+'result/%d/past_%s_%d_%d_%d_T%d_S%d_monthly'%(start_date, area, start_year, start_month, month_range, temp_mode, sub_mode)
