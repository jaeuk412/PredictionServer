...
def predict_sub_num(area, cat, target_year, target_mon, sub_mode):
    ...
    # return the true number of subscribers
        ...
        sub_data = pd.read_csv(folder_path + 'data/sub/%s_sub_%d' % (area, target_year), delim_whitespace=True)
...
def estimate_insu_per_cat(area, cat, target_year, target_month, temp_mode, sub_mode):
    ...
    try:
        true_insu_data = pd.read_csv(folder_path + 'data/insu/%s_insu_%d' % (area, target_year), delim_whitespace=True)
...
def conduct_prediction(area, start_year, start_month, month_range, temp_mode, sub_mode, start_date, user_pkey,
                       detectkey):
    '''
    ...
    daily_output_file = folder_path + 'result/%d/past_%s_%d_%d_%d_T%d_S%d_daily' % (
    user_pkey, area, start_year, start_month, month_range, temp_mode, sub_mode)
    ...
    monthly_output_file = folder_path + 'result/%d/past_%s_%d_%d_%d_T%d_S%d_monthly' % (
    user_pkey, area, start_year, start_month, month_range, temp_mode, sub_mode)
