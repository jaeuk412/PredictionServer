...
def write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_true_insu_list, category_list, mode):
	...
	f = open('daily_prediction_naju_%d'%(target_year), 'a+')
...
def predict_temp(raw_data, target_year, target_mon):
	...
	for tday in range(1, mon_days+1): 
		...
		prev_same_month = prev_same_year[prev_same_year['month']==target_mon] 
		target_day = prev_same_month[prev_same_month['date']==tday] 
		day_code = prev_same_month['day'].values[0]
		holiday_code =prev_same_month['holiday'].values[0]

		#prev_avg_temp.append( [np.average(prev_avg_temp_list, weights=weight_list), day_code, holiday_code] )
		prev_avg_temp.append( [np.average(prev_avg_temp_list, weights=weight_list), day_code])#, holiday_code] )
...
def predict_avg_insu_by_temp_regression(cat, raw_data, target_year, target_mon):
	...
	'''
	s_mon = (target_mon-1)%12
	if s_mon== 0:
		s_mon = 12
	e_mon = (target_mon+1)%12
	if e_mon== 0:
		e_mon = 12

	mon_list = [s_mon, target_mon, e_mon]
	'''
	...
	train_avg_day_list = [] # input list of curve fitting. 'x' of 'y=a*x+b'
	train_avg_holiday_list = [] # input list of curve fitting. 'x' of 'y=a*x+b'
	...
	for tyear in range(target_year-4, target_year):
		...
		for tmon in mon_list: 
			...
			for tday in range(1, mon_days+1): 
				...
				avg_temp = target_day['avgTemp'].values[0]
				day_code = target_day['day'].values[0]
				holiday_code = target_day['holiday'].values[0]

				#train_avg_temp_list.append([avg_temp, day_code, holiday_code])
				train_avg_temp_list.append([avg_temp, day_code])#, holiday_code])
	...
	xgb_model.fit(train_avg_temp_list, train_avg_insu_target) 

	# get the predicted temperature of the target_year.target_mon
	prev_avg_temp = predict_temp(raw_data, target_year, target_mon)
	predictions = xgb_model.predict(prev_avg_temp)

	return predictions


def func_mreg(X, a, b, c, d):
	x, y , z = X
	return a*x + b*y + c*z + d

def predict_avg_insu_by_mreg(cat, raw_data, target_year, target_mon):
	...
	for tday in range(1, mon_days+1): 
		...
		for tyear in range(target_year-4, target_year):
			...
			insu_target = target_day['insu_%s'%(cat)].values[0] 
			sub_target = target_day['sub_%s'%(cat)].values[0] 
			prev_avg_insu_target = insu_target/sub_target

			if tyear == target_year-4:
				xone.append(prev_avg_insu_target)
			elif tyear == target_year-3:
				xtwo.append(prev_avg_insu_target)
			elif tyear == target_year-2:
				xthr.append(prev_avg_insu_target)
			else:
				xtrue.append(prev_avg_insu_target)

	popt, pcov = curve_fit(func_mreg, xdata=(xone, xtwo, xthr), ydata=xtrue)

	xtwo = np.array(xtwo, dtype=np.float)
	xthr = np.array(xthr, dtype=np.float)
	xtrue = np.array(xtrue, dtype=np.float)
	prev_avg_insu = popt[0]*xtwo + popt[1]*xthr + popt[2]*xtrue + popt[3]
...
def predict_avg_insu_by_averaging(cat, raw_data, target_year, target_mon):
	...
	for tday in range(1, mon_days+1): 
		...
		if len(prev_avg_insu_target) > 0:
			...
			#prev_avg_insu.append( np.average(prev_avg_insu_target, weights=[x*0.5 for x in range(1, len(prev_avg_insu_target)+1)]) )
		# otherwise, use 0
		else:
...
def predict_avg_insu(cat, raw_data, target_year, target_mon):
	...
	'''
	# temp-based linear regression (i.e., input: temp, output: avg insu per sub)
	if cat in ['salesTwo', 'industry', 'heatCombined'] :

		return predict_avg_insu_by_temp_regression(cat, raw_data, target_year, target_mon)

	# averaging data of recent 3 years
	elif cat in ['house', 'houseCooking', 'houseJHeating', 'salesOne', 'bizHeating', 'bizCooling']:
...
def predict_avg_insu_pretest(cat, raw_data, target_year, target_mon, mode):
	'''
	This function estimates the average insu per subscriber of specified category of target_year.target_month through temmp-based linear regression.
	This function applies different prediction methods for target categories.
	Args:
...
import math
def find_matching_predict_function(history_file_name, target_year):
	'''
	This function finds a proper prediciton function for each category.
	Args:
	...
	naju_category = ['house', 'houseCooking', 'houseJHeating', 'salesOne', 'salesTwo', 'bizHeating', 'bizCooling', 'industry', 'heatFacility', 'heatCombined', 'CNG']
	...
	for cat in naju_category:
		...
		for tmon in range(1,13):
			...
			pred_avg_insu_temp_reg = predict_avg_insu_pretest(cat, raw_data, target_year, tmon, 1)
			pred_avg_insu_avg = predict_avg_insu_pretest(cat, raw_data, target_year, tmon, 2)
			pred_avg_insu_curve = predict_avg_insu_pretest(cat, raw_data, target_year, tmon, 3)
			...
			daily_cat_pred_insu_temp_reg = pred_avg_insu_temp_reg * pred_sub_num
			daily_cat_pred_insu_avg = pred_avg_insu_avg * pred_sub_num
			daily_cat_pred_insu_curve = pred_avg_insu_curve * pred_sub_num
			...
			cat_daily_mape_temp_reg, cat_monthly_mape_temp_reg, _ = calc_errors(daily_cat_true_insu, daily_cat_pred_insu_temp_reg)
			cat_daily_mape_avg, cat_monthly_mape_avg, _ = calc_errors(daily_cat_true_insu, daily_cat_pred_insu_avg)
			cat_daily_mape_curve, cat_monthly_mape_curve, _ = calc_errors(daily_cat_true_insu, daily_cat_pred_insu_curve)

			temp_reg_mape_list.append(cat_monthly_mape_temp_reg)
			avg_mape_list.append(cat_monthly_mape_avg)
			curve_mape_list.append(cat_monthly_mape_curve)
		print ("%.2f %.2f %.2f "%(np.mean(temp_reg_mape_list), np.mean(avg_mape_list), np.mean(curve_mape_list)))
...
def conduct_predict(history_file_name, target_year):
	...
	if math.isnan(float(tmp_cat[0])) == False:
		...
		for tidx in range(len(naju_category)):
			data += "%s_pred %s_true "%(naju_category[tidx], naju_category[tidx])
...
# find a proper prediction function for each category
#find_matching_predict_function('merged_naju_2014_2018', 2018)

# predict daily insu of naju of Y2018
conduct_predict('merged_naju_2014_2018', 2018)

# predict daily insu of naju of Y2019
conduct_predict('merged_naju_2014_2018', 2019)
