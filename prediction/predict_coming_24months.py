...
def predict_temp(area, target_year, target_mon):
    ...
    if not os.path.isfile(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, target_year)):
        tmp_f = open(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, target_year), 'w')
        tmp_f.write("year month date avgTemp maxTemp minTemp\n")
        need_to_write = 1
    # - the target file exists
    else:
        temp_tmp = pd.read_csv(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, target_year),
        ...
        # -- need to conduct prediction
            ...
            need_to_write = 1
            tmp_f = open(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, target_year), 'a')
    ...
    if need_to_write == 1:
        ...
        for tyear in range(target_year - 4, target_year):
            # read the temp data
            temp_data_tmp = pd.read_csv(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, tyear),
                                        delim_whitespace=True)
...
def predict_avg_insu_by_temp_regression(area, cat, target_year, target_mon):
    ...
    for tyear in range(target_year - 4, target_year):
        ...
        temp_data = pd.read_csv(folder_prediction_path + 'data/tmp_for_pred/temp/%s_temp_%d' % (area, tyear),
                                delim_whitespace=True)
...
def conduct_prediction(area, start_year, start_month, month_range, start_date, user_key, detectkey):
    '''
...
# /home/lee/PredictionServer/prediction/result/coming_naju_2019_1_24_daily
# /home/lee/PredectionServer/prediction/result/coming_naju_2019_1_24_daily'
