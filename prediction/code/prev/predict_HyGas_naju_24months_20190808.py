...
def write_past_prediction_to_file(daily_output_file, target_year, tmon, daily_pred_insu_list, daily_true_insu_list, category_list):
	'''
...
def write_future_prediction_to_file(daily_output_file, target_year, tmon, daily_pred_insu_list, category_list):
	'''
	This function writes daily prediction result into file.
	Args:
		daily_output_file: string, output file name
...
def check_and_prepare_data(area, op_mode, temp_mode, sub_mode):
	...
	print ("insu data is available up to %d.%d"%(insu_latest_year, insu_latest_month))
	...
	print ("sub data is available up to %d.%d"%(sub_latest_year, sub_latest_month))
	...
	print ("Both insu and sub data are available up to %d.%d"%(latest_year, latest_month))
...
import math
import datetime
def conduct_prediction_past(area, start_year, start_month, month_range, temp_mode, sub_mode):
	'''
	...
	for ridx in range(1, month_range+1):
		...
		write_past_prediction_to_file(daily_output_file, current.year, current.month, daily_pred_insu_list, daily_true_insu_list, category)

	monthly_f.close()


def conduct_prediction_future(area, start_year, start_month, month_range, temp_mode, sub_mode):
	'''
	...
	# - prepare the output file for daily prediction
	daily_output_file = './result/future_%s_%d_%d_%d_T%d_S%d_daily'%(area, start_year, start_month, month_range, temp_mode, sub_mode)
	daily_f = open(daily_output_file, 'a+') 
	data = "year month date "
	for tidx in range(len(category)):
	...
	# - prepare the output file for monthly prediction
	monthly_output_file = './result/future_%s_%d_%d_%d_T%d_S%d_monthly'%(area, start_year, start_month, month_range, temp_mode, sub_mode)
	monthly_f = open(monthly_output_file, 'a+') 
	data = "year month "
	for tidx in range(len(category)):
		data += "%s_pred "%(category[tidx])
	...
	for ridx in range(1, month_range+1):
		...
		for cat in category:
			...
			# estimate daily insu per category
			est_insu, _ = estimate_insu_per_cat(area, cat, current.year, current.month, temp_mode, sub_mode)
		...
		# write daily prediction results into a file
		write_future_prediction_to_file(daily_output_file, current.year, current.month, daily_pred_insu_list, category)
...
def main(area, op_mode, temp_mode, sub_mode):
	...
	# step 2: Conduct predictions
	if op_mode == 1:
		conduct_prediction_past(area, start_year, start_month, month_range, temp_mode, sub_mode)
	elif op_mode == 2:
		conduct_prediction_future(area, start_year, start_month, month_range, temp_mode, sub_mode)
