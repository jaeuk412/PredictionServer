...
def predict_avg_insu_by_averaging(cat, raw_data, target_year, target_mon):
	...
	for tday in range(1, mon_days+1): 
		...
		if len(prev_avg_insu_target) > 0:
			if cat == 'WAY':
				prev_avg_insu.append( np.average(prev_avg_insu_target, weights=[x for x in range(1, len(prev_avg_insu_target)+1)]) )
			else:
...
conduct_predict('./data/insu_sub/merged/merged_naju_2014_2018', 2018)
