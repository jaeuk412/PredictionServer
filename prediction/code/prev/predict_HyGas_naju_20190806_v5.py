...
def predict_temp(area, target_year, target_mon, temp_mode):
	...
	if temp_mode == 0:
		# manipulate weight for averaging temperature of recent 4 years. Note that winter of Y2017 and Y2018 was colder than normal
...
def check_and_prepare_data(area, start_year, start_month, month_range, temp_mode, sub_mode):
	...
	# examine a list of years to be considered
	year_list = []
	...
	for current_year in year_list:
		...
		if temp_mode == 1:
			try:
				target_temp_data = pd.read_csv('data/temperature/%s_temp_%d'%(area, current_year), delim_whitespace=True)
			except FileNotFoundError:
		...
		if sub_mode == 1:
			print ("")
			# TODO: HyGAS should prepare the file
