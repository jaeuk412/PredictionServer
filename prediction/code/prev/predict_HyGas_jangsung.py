...
def write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_true_insu_list, category_list, mode):
	...
	f = open('daily_prediction_jangsung_%d'%(target_year), 'a+')
...
def conduct_predict(history_file_name, target_year):
	...
	jangsung_category = ['house', 'houseJHeating', 'salesOne', 'salesTwo', 'bizHeating', 'industry']
	...
	if math.isnan(float(tmp_cat[0])) == False:
		...
		f = open('daily_prediction_jangsung_%s'%(target_year), 'w')
		# - write column name first
		data = "year month date "
		for tidx in range(len(jangsung_category)):
			data += "%s_pred %s_true "%(jangsung_category[tidx], jangsung_category[tidx])
		...
		for tmon in range(1,13):
			...
			write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_true_insu_list, jangsung_category, 1)
	...
	# when there is no true label of target_year (e.g., 2019)
		...
		data = "year month date "
		for tidx in range(len(jangsung_category)):
			data += "%s "%(jangsung_category[tidx])
		data += "pred_sum\n"
		f.write(data)
		f.close()

		# true monthly insu volume of Y2019 of jangsung area
		...
		for tmon in range(1,7):
			...
			# write prediction result into file
			write_prediction_to_file(target_year, tmon, daily_pred_insu_list, daily_pred_insu_list, jangsung_category, 2)
...
# predict daily insu of jangsung of Y2018
conduct_predict('merged_jangsung_2014_2018', 2018)
