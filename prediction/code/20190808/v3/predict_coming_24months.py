...
def write_prediction_to_tmp_file(area, target_year, tmon, daily_pred_insu_list, daily_pred_sub_list, category_list):
	...
	if not os.path.isfile('data/tmp_for_pred/insu/%s_insu_%d'%(area, target_year)):
		insu_f = open('data/tmp_for_pred/insu/%s_insu_%d'%(area, target_year),'w') 
		...
		insu_f = open('data/tmp_for_pred/insu/%s_insu_%d'%(area, target_year),'a')

	if not os.path.isfile('data/tmp_for_pred/sub/%s_sub_%d'%(area, target_year)):
		sub_f = open('data/tmp_for_pred/sub/%s_sub_%d'%(area, target_year),'w') 
		data = "year month date " 
		for tidx in range(len(category_list)): 
		...
		sub_f = open('data/tmp_for_pred/sub/%s_sub_%d'%(area, target_year),'a')
...
def predict_temp(area, target_year, target_mon):
	...
	if not os.path.isfile('data/tmp_for_pred/temperature/%s_temp_%d'%(area, target_year)):
		tmp_f = open('data/tmp_for_pred/temperature/%s_temp_%d'%(area, target_year),'w')
		tmp_f.write("year month date avgTemp maxTemp minTemp\n")
		need_to_write = 1
	# - the target file exists
	else: 
		temp_tmp = pd.read_csv('data/tmp_for_pred/temperature/%s_temp_%d'%(area, target_year), delim_whitespace=True) 
		...
		# -- need to conduct prediction
			...
			need_to_write = 1
			tmp_f = open('data/tmp_for_pred/temperature/%s_temp_%d'%(area, target_year),'a')
	...
	if need_to_write == 1:
		...
		for tyear in range(target_year-4, target_year):
			# read the temp data
			temp_data_tmp = pd.read_csv('data/tmp_for_pred/temperature/%s_temp_%d'%(area, tyear), delim_whitespace=True)
...
def predict_sub_num(area, cat, target_year, target_mon):
	...
	for tyear in range(target_year-4, target_year):
		...
		sub_data_tmp = pd.read_csv('data/tmp_for_pred/sub/%s_sub_%d'%(area, tyear), delim_whitespace=True)
		# merge the read data into one data for easy processing
		if tyear == target_year-4:
...
def predict_avg_insu_by_temp_regression(area, cat, target_year, target_mon):
	...
	for tyear in range(target_year-4, target_year):
		...
		temp_data = pd.read_csv('data/tmp_for_pred/temperature/%s_temp_%d'%(area, tyear), delim_whitespace=True)

		# read the insu data
		insu_data = pd.read_csv('data/tmp_for_pred/insu/%s_insu_%d'%(area, tyear), delim_whitespace=True)

		# read the subscriber data
		sub_data = pd.read_csv('data/tmp_for_pred/sub/%s_sub_%d'%(area, tyear), delim_whitespace=True)

		# read the date data
		date_data = pd.read_csv('data/tmp_for_pred/date/date_info_Y%d'%(tyear), delim_whitespace=True)
	...
	date_data = pd.read_csv('data/tmp_for_pred/date/date_info_Y%d'%(target_year), delim_whitespace=True)
...
def predict_avg_insu_by_averaging(area, cat, target_year, target_mon):
	...
	for tyear in range(target_year-3, target_year):
		...
		# read the insu data
		insu_data_tmp = pd.read_csv('data/tmp_for_pred/insu/%s_insu_%d'%(area, tyear), delim_whitespace=True)
		# - merge the read data into one data for easy processing
		...
		# read the date data
		date_data_tmp = pd.read_csv('data/tmp_for_pred/date/date_info_Y%d'%(tyear), delim_whitespace=True)
		# - merge the read data into one data for easy processing
		if tyear == target_year-3:
	...
	target_date_data = pd.read_csv('data/tmp_for_pred/date/date_info_Y%d'%(target_year), delim_whitespace=True)
...
import math
import datetime
def conduct_prediction(area, start_year, start_month, month_range):
	'''
	...
	daily_output_file = './result/coming_%s_%d_%d_%d_daily'%(area, start_year, start_month, month_range)
	...
	monthly_output_file = './result/coming_%s_%d_%d_%d_monthly'%(area, start_year, start_month, month_range)
	...
	for ridx in range(1, month_range+1):
		...
		write_prediction_to_tmp_file(area, current.year, current.month, daily_pred_insu_list, daily_pred_sub_list, category)
