...
def get_values_name(input_value):
    result_depth1 = []
    keyname_list = []
    ...
    print("get_value_nmae_len: ",len(keyname_list))
    return keyname_list
...
def circle(file_name, tar_frame):
    ...
    for json_raw in range(len(df.index)):
        ...
        try:
            ...
            tar_name, get_tar_list = get_feature_ext(json_data)
            # print('--------------------3--------------------')
            # print(get_tar_list)
            # print("tar_list_len",len(get_tar_list))
            tar_list.append(get_tar_list)
            # print("tar_name_len: ",len(tar_name))
            # print(len(tar_list))
            # print('--------------------4--------------------')
        except ValueError:
            print('--------------------4--------------------')
    ...
    try:
        ...
        tar_box_to_save = (list2pd(tar_list, df, tar_name)).reset_index(drop=True)
        # tar_box_to_save = (list2pd(get_tot_list, tar_list, df))
        print('--------------------6--------------------')
        if variable_check(keyname_list) == True:
            print('--------------------7--------------------')
            tar_save(tar_box_to_save, keyname_list)
            print('--------------------8--------------------')
