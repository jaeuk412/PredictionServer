...
test_value =  {'bms': {'802': ['A', 'AChaMax', 'ADisChaMax', 'SoC', 'SoH', 'V'], '1803': ['VCell', 'VCellMax', 'VCellMin']},
...
import pandas as pd
import requests
jfile = '/home/uk/PredictionServer/prediction2/JSONparser/jsondatafile/32/2018/12/backup_2018-12-17'


# json_data = open(jfile, "r", encoding="UTF-8").read()
# json_data = requests.get(json_data).json()

# data = json.loads(json_data)

# print(json_data)
# print(depth(json_data))

# with open(jfile, 'r') as json_file:
#     json_file = json_file.readlines()
#     # json_file = json_file[0].replace('\x00', '')
#     print(type(json_file))
#     r = requests.get(json_file).text
...
for index, row in df1.iterrows():
    ...
    jso = json.loads(date)
    print("jso: ",jso)
    # print("vv: ",jso.get('bms'))
    jso1=jso.get('meter--load')
    if not jso1:
        okwod.update(jso1)
        count += 1
...
print(jso)
print(sorted(jso.keys()))



jso1 = {'1805': {'NRackFault': 0, 'NRackOn': 14, 'NRackFanOn': 1, 'Hb': 0, 'NRackCellBal': 0, 'NRackDsOn': 14, 'NRack': 14, 'NRackWarn': 0}, '1825': {}, '1803': {'VCell': 3.602, 'VCellMinMaxLoc': {'max': 1, 'min': 12}, 'TmpMin': 21.5, 'TmpMinMaxLoc': {'max': 8, 'min': 1}, 'TmpMax': 26.5, 'VCellMax': 3.61, 'Tmp': 24, 'VCellMin': 3.588}, '802': {'AChaMax': 1989, 'A': 0, 'StVnd': 'NPS', 'SoH': 100, 'ADisChaMax': 1989, 'EvtVnd3': [], 'SoC': 26, 'EvtVnd2': [], 'WDisChaMax': 1705200, 'EvtVnd1': [], 'WChaMax': 1705200, 'EvtVnd4': [], 'V': 857.3, 'StEvtVnd': {'low soc alram': 'NORMAL', 'fault': 'NORMAL', 'contactor control': 'NORMAL', 'charge status': 'IDLE', 'under balance': 'NORMAL', 'comm error': 'NORMAL', 'online': 'ONLINE', 'warning': 'NORMAL', 'high soc alram': 'NORMAL'}}, '1823': {'Sleep': 0, 'ManMode': 0, 'Bsc': 0, 'Hb': 0, 'Rst': 'NONE', 'Contactor': 'NONE', 'StController': 'SHUTDOWN'}}
...
for key1 in sorted(jso1.keys()):
    ...
    # print("key1: ", key1)
    key1_value = jso1.get(key1)
    # print("key1name: ", key1)
    # print("key1_value: ",key1_value)
    result, depth1, keyname = search_values_name(key1_value, key_edit)
    ...
    for i in list_ff:
        ...
        try:
            ...
            if testtarget:
                ...
                final_value_result = final_value_result + [testtarget]
            ...
            pass
#
# print(len(final_value_result))
# print(len(final_name_result))
